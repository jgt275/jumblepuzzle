...
def find_anagrams(jumbled_word):
    ...
    freq_dict_subset = {k:v for k,v in word_freq.items() if len(k) == word_len}
    for word in freq_dict_subset:
        word_lowercase = word.lower()
        if (word_lowercase != jumbled_word) and (Counter(jumbled_word) == Counter(word_lowercase)):
            anagrams.append(word_lowercase)
    return anagrams
...
def find_likely_anagram(jumbled_word):
    ...
    all_anagrams = find_anagrams(jumbled_word)
    if len(all_anagrams)==1:
        return all_anagrams
    elif len(all_anagrams)>1:
        freq_dict_subset_gt0= {k: word_freq[k] for k in all_anagrams if word_freq[k]>0}
        ...
        #condition when all the returned anagrams have zero frequency
            freq_dict_subset = {k: word_freq[k] for k in all_anagrams}
...
if __name__=="__main__":
    ...
    puzzle_df = puzzle_df.select('*',anagram_func(f.lower(f.col('word'))).alias('anagrams'))
    ...
    puzzle_df = puzzle_df.groupby("id","final_word_letters").agg(f.collect_list('circled_letters').alias('agg_circled_letters'))
    ...
    puzzle_df = puzzle_df.select('*', cartesian_word_join_func(f.col('agg_circled_letters')).alias('final_phrase_jumbled_letters'))
